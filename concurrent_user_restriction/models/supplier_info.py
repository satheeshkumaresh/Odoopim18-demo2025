...
class supplierInfo(models.Model):
    ...
    def can_edit_session(self):
        if not self.env.user.has_groups('pim_ext.group_sup_user_write') or not self.id:
            return 2
    ...
    def check_lock(self):
        ...
        if self.env.user.has_groups('pim_ext.group_sup_user_write'):
    ...
    def clear_edit_session(self):
        ...
        if self.env.user.has_group('pim_ext.group_sup_user_write'):
            self_session = request.session.sid
            session_details = self.env['session.info'].search(
                [('session_id', '=', self_session), ('supplier_id', '=', self.id)])
    ...
    def get_editing_session(self, session):
        sessions = self.env['session.info'].search([('supplier_id', '=', self.id), ('active_state', '=', True)],
    ...
    @api.onchange('name', 'street', 'street2', 'city_id', 'state_id', 'country_id', 'zip','website',
                  'notes', 'contact_ids')
    def edit_mode(self):
        if self.env.user.has_groups('pim_ext.group_sup_user_write'):
            if self.editing:
