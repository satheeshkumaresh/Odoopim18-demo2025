...
class FamilyAttribute(models.Model):
    ...
    def can_edit_session(self):
        if not self.env.user.has_groups('pim_ext.group_family_attribute_user_write') or not self.id:
            return 2
    ...
    def check_lock(self):
        ...
        if self.lock_time and self.env.user.has_groups('pim_ext.group_family_attribute_user_write'):
    ...
    def clear_edit_session(self):
        ...
        if self.env.user.has_group('pim_ext.group_family_attribute_user_write'):
            self_session = request.session.sid
            # self._cr.execute(
            #     f"""
            #                     SELECT
            ...
            #
            #                 """,
            #     [self_session, self.id],
            # )
            # session_details = self._cr.fetchall()[0][0]
            # session_details = self.env['session.info'].browse(session_details)
            # print("session_details",session_details)
            session_details = self.env['session.info'].search(
                [('session_id', '=', self_session), ('family_id', '=', self.id)])
    ...
    def get_editing_session(self, session):
        sessions = self.env['session.info'].search([('family_id', '=', self.id), ('active_state', '=', True)],
    ...
    @api.onchange('name', 'supplier_id', 'brand_id', 'buyer_id', 'manufacture_id', 'availability',
                  'gift', 'swatch', 'family_logo', 'family_msds', 'family_msds_document',
                  'family_confirmation_letter', 'taxonomy_ids','product_family_ids','attribute1_id','attribute2_id','attribute3_id')
    def edit_mode(self):
        if self.env.user.has_groups('pim_ext.group_family_attribute_user_write'):
            if self.editing:
