...
class ProductBrand(models.Model):
    ...
    def can_edit_session(self):
        if not self.env.user.has_groups('pim_ext.group_brand_user_write') or not self.id:
            return 2
    ...
    def check_lock(self):
        ...
        if self.env.user.has_groups('pim_ext.group_brand_user_write'):
    ...
    def clear_edit_session(self):
        ...
        if self.env.user.has_group('pim_ext.group_brand_user_write'):
            self_session = request.session.sid
            session_details = self.env['session.info'].search([('session_id','=',self_session),('brand_id','=',self.id)])
    ...
    def save_session(self, self_session):
        ...
        if not session_details:
            session_details = self.env['session.info'].create({'session_id':self_session,
                                             'brand_id':self.id,
    ...
    def get_editing_session(self,session):
        sessions = self.env['session.info'].search([('brand_id', '=', self.id), ('active_state', '=', True)],
        ...
        if self.env.user.has_groups('pim_ext.group_brand_user_write'):
            if self.editing:
                if not self.edit_session_name:
                    ...
                    raise ValidationError("Another user is editing. Please reload and try again.")
        return super().action_add_contact()
    ...
    @api.onchange('name', 'website', 'abbreviation','portal_username','portal_url','portal_password',
                  'notes','description','brand_logo','collateral_image','msds','msds_document',
                  'confirmation_letter','contact_ids')
    def edit_mode(self):
        print("check editing", request.session.sid, self.edit_session_name, self.editing)
        if self.env.user.has_groups('pim_ext.group_brand_user_write'):
            if self.editing:
                if not self.edit_session_name:
                    raise ValidationError("Another user is editing. Please reload and try again.")
            ...
            raise ValidationError("You dont have the right to edit Brand.")
