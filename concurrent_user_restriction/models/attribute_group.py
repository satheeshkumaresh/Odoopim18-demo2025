...
class AttributeGroup(models.Model):
    ...
    def get_editing_session(self, session):
        sessions = self.env['session.info'].search([('attribute_group_id', '=', self.id), ('active_state', '=', True)],
    ...
    @api.onchange('name', 'active', 'default_group', 'channel_id', 'attribute_family_id', 'attribute_text_ids',
                  'attribute_ids')
    def edit_mode(self):
        if self.env.user.has_groups('pim_ext.group_attribute_user_write'):
            if self.editing:
